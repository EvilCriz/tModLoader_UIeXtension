...
`UIExDockPanel` - это контейнер компоновки, который **последовательно** пристыковывает дочерние элементы к одной из пяти сторон доступной области:
- `Left`
- `Right`
- `Top`
- `Bottom`<br>

Оставшееся место также считается отдельной стороной: `Fill`.<br>
Она занимает все оставшееся доступное пространство, после вычисления предыдущих сторон.<br>

`UIExDockPanel` можно условно представить как два уровня компоновки:
1. Первый уровень: последовательное выделение области `Left`, `Right`, `Top`, `Bottom` и `Fill`;
2. Второй уровень: каждый сторона самостоятельно компонует пристыкованные к ней элементы по правилам, практически идентичным `UIExStackPanel`.<br>
...
2. Ширина сторон `Left` и `Right`: ширина самой большой внешней области из пристыкованных к этой стороне элементов.
3. Ширина сторон `Top` и `Bottom`: ширина свободного места в контейнере.
4. Высота сторон `Top` и `Bottom`: высота самой большой внешней области из пристыкованных к этой стороне элементов.<br>

После того как область первой добавленной стороны рассчитана, данная область исключается из свободного пространства контейнера.<br>
...
Старайтесь при этом предсказывать, как этот результат будет выглядеть.<br>
...
`UIExDockPanel` также имеет «сторону» `Fill`. Это сторона, которое занимает всю оставшуюся область после вычисления сторон `Left`, `Right`, `Top`, `Bottom`.<br>
Пример того, как это выглядит вы можете посмотреть ниже.<br>
Порядок добавления элементов с `Side == Fill` не влияет на порядок вычисления сторон.<br>
`Fill` всегда вычисляется после `Left`, `Right`, `Top` и `Bottom` и занимает все оставшееся пространство.<br>
...
- `AlignItems`: выравнивание элементов по поперечной оси (работает для всех сторон; можно переопределить данное значение для конкретной стороны и/или конкретного элемента).<br><br>

- У `Left`, `Right`, `Top` и `Bottom` направление компоновки однозначно определяется самой стороной.<br>
- `Fill` не имеет фиксированного направления, поэтому его ориентация задается через `StyleLayout.Orientation`.<br>
...
- Поле `AlignSelf` переопределяет значение `AlignItems` для конкретного вложенного элемента. Значение `Auto` означает, что значение будет управляться контейнером/стороной.
- Поле `Margin` учитывается при компоновке элемента и при определении размера занимаемой им внешней области.
...
- `AlignItems`: выравнивание элементов по поперечной оси. Переопределяет значение `AlignItems` класса стиля контейнера для этой конкретной стороны. Значение `Auto` означает, что значение будет управляться контейнером.
...
<h2 align="center">Пример использования <i>UIExDockPanel</i></h2>
...
<p align="center">
  ...
  <br>
  <em>Рис. 1.4. Визуальное отображение работы кода</em>
</p>
...
- 1 элемент со значением `Side == UIExSide.Bottom;`
- 4 элемента со значением `Side == UIExSide.Left;`
- 3 элемента со значением `Side == UIExSide.Top;`
- 5 элементов со значением `Side == UIExSide.Right;`
- 1 элемент со значением `Side == UIExSide.Fill;`
...
<table align="center">
  ...
  <tr>
    ...
    <td>Максимальная внешняя ширина элементов</td>
    <td>Вся свободная высота</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><code>Top</code></td>
    <td>Вся свободная ширина</td>
    <td>Максимальная внешняя высота элементов</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>Вся оставшаяся ширина</td>
    <td>Вся оставшаяся высота</td>
  </tr>
</table>
...
Этот код сообщает компоновщику, что ему нужно выводить вспомогательные линии размером `3px` и указанного цвета.<br>
...
1. Сторона **НЕ** переопределяет одно из этих значений (об этом ниже).
2. Элемент **НЕ** переопределяет одно из этих значений (об этом ниже).

----

Далее в коде получается и сохраняется стиль нижней стороны:
```csharp
SideStyle sideBottom = styleLayoutDock.SideBottom;
```
...
Как видно, данный класс хранит в себе стили сторон. Класс `SideStyle` выглядит следующим образом:
```csharp
public class SideStyle : Base.StyleBase
{
    public UIExThickness Padding = default(UIExThickness);
...
4. `Padding` представляет собой тип `UIExThickness` и определяет отступы от **области** стороны до **внутренней области** стороны. Более подробно про области рассказано в статье: *класс «`UIExStackPanel`». Основы компоновки*.
...
1. Создается 1 квадратная кнопка с размером ребра 50px. Текст внутри кнопки: `b` (bottom) + ее индекс.
...
1. Сохраняется класс `StyleLayoutChild` с помощью метода расширения класса `UIElement`: `StyleLayoutChild()`.
...
Кроме элементов `Fill`.<br>
Порядок добавления элементов с `Side == Fill` не влияет на порядок вычисления сторон.<br>
`Fill` всегда вычисляется после `Left`, `Right`, `Top` и `Bottom` и занимает все оставшееся пространство.<br>
То есть если мы переместим код, отвечающий за добавление элементов в оставшееся пространство - в самое начала, результат компоновки не изменится.
...
В остальном правила расположения элементов аналогичное остальным сторонам.
  ...
  <tr>
    ...
    <td align="left">
      <a 
        ...
        Раздел II: Элементы компоновки. Часть 6: «Класс UIExGridPanel»
      </a>
    </td>
  </tr>
  <tr>
