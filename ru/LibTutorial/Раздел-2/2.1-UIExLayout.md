...
Это дополнительная информация для лучшего понимания, как работает `Padding`, `Margin` и `Stretch` при работе с компоновщиками.<br>
Если вы не считаете несообразным тратить время на понимание данной части статьи - вы можете ее пропустить.<br>
...
`Left` и `Top` - это смещение элемента относительно внутреннего размера родительского элемента. 
...
1. Берется размер внутренней области свободного пространства (выделенное компоновщиком) для размещения.
2. Исходя из размера внутренней области выделенного пространства и значений (`Width`, `MinWidth`, `MaxWidth`, `Height`, `MinHeight`, `MaxHeight`) определяется размер элемента.
3. Исходя из смещения внутренней области выделенного пространства и значений `Top` и `Left` определяется текущее положение элемента (относительно начала экрана, а не родительского элемента).
Например:
```csharp
public class ExamplePanelUI : UIState
{
    public override void OnInitialize()
	{
		...
		var style = innerPanel.StyleLayoutChild().Canvas();
		innerPanel.MaxWidth.Pixels = 50f;
		innerPanel.Width.Pixels = 100f;
		innerPanel.Height.Pixels = 0.5f;
		innerPanelStyle.Left.Pixels = 50f;
		innerPanelStyle.Top.Pixels = 50f;
		Append(panel);
	}
}
```
...
И тогда область `innerPanel`:
1. `X`: `область-canvas-x` + `innerPanelStyle.Left`.
2. `Y`: `область-canvas-y` + `innerPanelStyle.Top`.
3. `Width`: 50px;
4. `Height`: `область-canvas-Height * 0.5 = 400px * 0.5 = 200px`.

Внешний размер элемента определяется (у компоновщиков данной библиотеки) следующим алгоритмом:
1. 
2. Исхода из 
