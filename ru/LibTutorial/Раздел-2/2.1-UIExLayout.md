...
text.StyleLayoutChild().WithoutLayout = true;
...
В данной подразделе статьи дана довольно глубокая информация о размещении элементов.<br>
Возможно больше, чем вам нужно для использования данной библиотеки.<br>
Эта информация нужна в 4 случаях:
...
3. Вы не понимаете (и хотели бы понять) как именно `Margin` влияет на компоновку и влияет на `Stretch`.
4. Вы хотели бы изучить данный вопрос просто для понимания внутреннего процесса компоновки.
Если вы не считаете нецелесообразным тратить время на понимание данной части статьи - вы можете ее пропустить.<br>
Если вы решили изучить данную часть статьи - рекомендуется ее перечитать дважды.<br>
Так как невозможно объяснить какую-либо из 3 областей не затрагивая другую.<br>
Пока у вас не будет поверхностного понимания каждой из них - вы не сможете понять глубоко никакую.<br>

Каждый элемент имеет 3 области (границы + размер):
...
1. `X-области`: отступ по ширине от начала экрана внутренней области родителя + `Left-элемента`.
2. `Y-области`: отступ от по высоте от начала экрана внутренней области родителя + `Top-элемента`.
3. `Width-области`: если `Width < MinWidth`: `Width-области = MinWidth;` если `Width > MaxWidth`: `Width-области = MaxWidth;` иначе `Width-области = Width`.
4. `Height-области`: если `Width < MinHeight`: `Height-области = MinHeight;` если `Height > MaxHeight`: `Height-области = MaxHeight;` иначе `Height-области = Height`.<br>
Т.е. область элемента - это его смещение от начала экрана + размер элемента, ограниченный `Min` и `Max`.
...
Внешний размер влияет только на то, сколько по мнению компоновщик занимает пространства элемент.<br>
...
И тут сделаем паузу, чтобы сделать важную заметку по Marign:
1. Margin.Top и Margin.Left - увеличивают внешнюю область и **смещают X, Y** элемента при компоновке.
2. Margin.Right и Margin.Bottom - увеличивают внешнюю область элемента и **НЕ** смещают X, Y элемента при компоновке.
...
1. `outer-X`: `X + PaddingLeft`.
2. `outer-Y`: `Y + PaddingTop`.
3. `outer-Width`: `Width - PaddingLeft - PaddingRight`.
4. `outer-Height`: `Height - PaddingTop + PaddingBottom`.<br>
...
public override void OnInitialize()
{
    ...
    canvas.PaddingLeft = 10f;
    canvas.PaddingTop = 20f;
    canvas.PaddingRight = 30f;
    canvas.PaddingBottom = 40f;
...
В данном примере `canvas` занимает 100% ширины и высоты экрана. Это область, которая так одинакова с внешней областью.<br>
Внутренняя область `canvas:
1. `inner-X`: `X == 0` - `PaddingLeft == 15px`.
2. `inner-Y`: `Y == 0` - `PaddingTop == 25px`.
3. `inner-Width`: `Width == screeWidth` - `PaddingLeft == 15px`- `PaddingRight == 35px`.
4. `inner-Height`: `Height == screenHeight` - `PaddingTop == 25px` - `PaddingBottom == 45px`.<br>

Результат:
1. `inner-X`: `15px`.
2. `inner-Y`: `25px`.
3. `inner-Width`: `screeWidth - 50px`.
4. `inner-Height`: `screenHeight - 70px`.<br>

Область (не внешняя или внутренняя) `innerPanel` имеет 100% ширины и 100% высоты контейнера. Но это 100% от внутреннего размера.<br>
Т.е. область `innerPanel` равна внутренней области `canvas`.<br>
Но это лишь до того, как в расчету вступит `Margin`.<br>
...
4. `outer-Height`: `canvas-inner-height + innerPanelStyle.Margin.Top + innerPanelStyle.Margin.Bottom`
