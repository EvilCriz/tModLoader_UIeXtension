...
canvas.StyleLayout.JustifyContent = UIExAlignment.Center;
```
В этом случае `canvas` не узнает, что его `JustifyContent` изменился. Для этого после применения новый стилей вызове метод `RefreshStyles()` у компоновщика или любого из вложенных в него элементов.
...
`RefreshStyles()` также можно вызывать до изменения стилей. Это для компоновщика не имеет значения, главное, чтобы этот метод был вызван **после** добавления компоновщика внутрь другого элемента.<br>
...
- Метод `RefreshStyles()` говорит компоновщику: при следующей попытке компоновки забудь о том, что ты пересчитывал свои стили и стили вложенных элементов.
