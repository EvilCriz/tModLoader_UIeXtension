...
public class StyleLayoutChild : IStyleLayoutChild, IStyleCopyManager<StyleLayoutChild>
{
    ...
    public StyleDimension JustifyOffset = StyleDimension.Empty;
    public StyleDimension AlignOffset = StyleDimension.Empty;

	public bool WithoutLayout = false;

    public void Set(Enums.Alignment justifySelf, Enums.Alignment alignSelf, Thickness margin, StyleDimension justifyOffset, StyleDimension alignOffset, bool withoutLayout = true) { /* ... */ }
...
Про `Margin`, `JustifyOffset` и `AlignOffset` мы подробнее поговорим ниже.<br>
...
Если какие-то из значений `Margin` указаны в процентах - это будет считаться процентом от размера всего контейнера компоновщики.
<br>

`JustifyOffset`, `AlignOffset` - смещение относительно основной/поперечной оси. 
