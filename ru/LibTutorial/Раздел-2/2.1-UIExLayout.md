...
```csharp
public void SetShowLayoutLinesForTreeBranch(bool state, float? thickness = null, Color? color = null) { /* ... */ }
public void SetShowLayoutLinesForTree(bool state, float? thickness = null, Color? color = null) { /* ... */ }
```

- `SetShowLayoutLinesForTreeBranch` устанавливает переданные (not null) значения для текущего элемента и всех его потомков, которые наследуют класс `UIExLayout`
- `SetShowLayoutLinesForTree` ищет корневой родительский элемент и вызывает для него метод `SetShowLayoutLinesForTreeBranch`
