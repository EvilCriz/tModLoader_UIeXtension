...
1. Внешний уровень последовательно выделяет области `Left`, `Right`, `Top`, `Bottom` и `Fill`;
2. Каждый сторона самостоятельно компонуются по правилам, практически идентичным `UIExStackPanel`.<br>
