...
1. `Top`: аналогичен `UIElement.Top`. Смещение элемента от относительно верхней части контейнера. Если значение 10px - элемент будет расположен на 10px ниже верней части контейнера.
2. `Left`: аналогичен `UIElement.Left`. Смещение элемента от относительно левой части контейнера.
3. `Bottom`: нет имеет аналога в `UIElement`. Смещение элемента от относительно нижней части контейнера.
4. `Right`: нет имеет аналога в `UIElement`. Смещение элемента от относительно правой части контейнера.

- `Top` и `Bottom` замещают друг друга. Это значит: если `Top.Set(10f, 0f)` и `Bottom.Set(10f, 0f)` - то смещение элемент будет 0.
...
public class ExamplePanelUI : UIState
{
    ...
    public override void OnActivate()
    {
        ...
        {
            styles[i] =
                canvas.GetOrRegisterStyleLayoutChild(cubs[i]);

            stylesCanvas[i] =
                canvas.GetOrRegisterStyleLayoutChildCanvas(cubs[i]);
        }
...
Кубы 0-6 расположены: 
`0`: верх по горизонтали; лево по вертикали;
`1`: верх по горизонтали; центр по вертикали;
`2`: верх по горизонтали; право по вертикали;
`3`: лево по горизонтали; центр по вертикали;
`4`: по центру экрана;
`5`: право по горизонтали; центр по вертикали.
`6`: растянута по горизонтали; низ по вертикали
```csharp
styles[0].Set(
    justifySelf:    UIExAlignment.Start, 
    alignSelf:      UIExAlignment.Start);
...
У элемента 6 (`style[6]`) поле `justifySelfl == UIExAlignment.Stretch`. Это означает, что оно выравнивается по всему размеру главной оси. В данном случае - по всей ширине. 
...
styles[12].Margin = new UIExThickness(
    ...
    right: 100f);
```
Этот пример уже более глубокий. Вы можете увидеть данный элемент на один выше и на два левее центрального (4-го) элемента.
Вы можете заметить, что аналогичного эффекта можно было бы достичь записью:
```csharp
styles[12].Bottom.Pixels = 50f;
styles[12].Right.Pixels = 50f;
```
...
Для всех прочих настроек нет никакой разницы, что именно использовать для смещения.
