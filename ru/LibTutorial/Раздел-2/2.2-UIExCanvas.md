...
В отличие от стандартных инструментов **tModLoader** для расположения элементов по координатам `Left` и `Top`, данная библиотека предлагает использовать 4 координаты.
1. `Top`: смещение элемента относительно верхней части контейнера. Если значение 10px - элемент будет расположен на 10px ниже верней части контейнера.
...
- Ниже мы добавляем `canvas` в главную панель `panel.Append(canvas);`, что означает, что `canvas` будет компоновать элементы по всей главной панели, то есть по всему созданному интерфейсу.
...
Обратите внимание, что размер внутри метода задается с помощью стиля `StyleLayoutChild`.<br>
...
Это один из двух элементов, у которого вспомогательные линии не совпадают с реальными элементами. И главную роль тут играет `Margin`.<br>
...
Результат (предварительный): (`c50px` - условное обозначение размера не растянутого ребра куба (размер: `50px`))
...
2. Позицию указанную в процентах и которая является отрицательной. Данный элемент центрирован, поэтому -0.5f сместит элемент вправо на 25% контейнера (50% от центрированной точки отсчета координат до левого края контейнера). Подробнее будет при разборе куба с индексом 14.
...
Как можно заметить, в отличие от всех прочих элементов, у элемента 6 и 12 вспомогательный прямоугольник нарисован с отступом.<br>
  ...
  <tr>
    ...
    <td align="left">
      <a 
        ...
        Раздел II: Элементы компоновки. Часть 1: «Класс UIExLayout»
      </a>
    </td>
  </tr>
  <tr>
