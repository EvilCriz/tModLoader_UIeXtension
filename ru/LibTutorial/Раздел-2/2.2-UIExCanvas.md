...
Чтобы вы могли использовать данный стиль, вам необходимо получить стиль `StyleLayoutChild` с помощью метода расширения `UIElement`: `StyleLayout()`.<br>
Затем у полученного стиля вызвать метод `Canvas()`.
```csharp
UIElement element = new();
StyleLayoutChild style = element.StyleLayout();
