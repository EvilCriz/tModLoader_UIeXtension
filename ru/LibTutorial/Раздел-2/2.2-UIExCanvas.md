...
`6`: растянута по горизонтали; низ по вертикали.<br>
```csharp
styles[0].Set(
    justifySelf:    UIExAlignment.Start, 
    alignSelf:      UIExAlignment.Start);
...
У элемента 6 (`style[6]`) поле `justifySellf == UIExAlignment.Stretch`. Это означает, что оно выравнивается по всему размеру главной оси. В данном случае - по всей ширине. 
...
stylesCanvas[15].AllowOverflowSelf = true;
```
В первом случае видно, что левое смещение установлено на 100 тысяч пикселей.<br>
