...
{
    public bool AllowChildOverflow = false;

    public void Set(bool allowChildOverflow) { /* .. */ }
    public void Copy(StyleLayoutContainerCanvas style) { /* .. */ }
    public StyleLayoutContainerCanvas GetCopy() { /* .. */ }
}
```
- `AllowChildOverflow` отвечает за то, могут ли элементы выйти за границу контейнера.
...
1. Добавляет смещение относительно выбранной координате.
2. Добавляет`AllowOverflowSelf`. Данное значение, если оно отлично от `null` переопределяет значение `AllowChildOverflow` контейнера `UIExCanvas` для этого конкретного элемента.<br>
...
`6`: растянут по горизонтали; низ по вертикали.<br>
```csharp
styles[0].Set(
    justifySelf:    UIExAlignment.Start, 
    alignSelf:      UIExAlignment.Start);
