...
* `Orientation`: визначає головну вісь контейнера, за якою будуть вирівнюватися елементи (усередині ліній).
* `JustifyContent`: вирівнювання елементів за головною віссю (усередині вже сформованих ліній).
* `AlignItems`: вирівнювання елементів за поперечною віссю (усередині вже сформованих ліній).
...
* Поле `JustifySelf` перевизначає значення `JustifyContent` контейнера для конкретного вкладеного елемента. Для дочірніх елементів ігнорує будь-які значення, крім значення `Stretch`.
...
* `AlignLines`: визначає вирівнювання всіх ліній по поперечній осі.
* `ReverseWithinLine`: якщо це значення `true` — елементи всередині кожної лінії будуть розташовуватися в реверсивному порядку.
* `ReverseAll`: якщо це значення `true` — всі елементи будуть розташовуватися в реверсивному порядку (і будуть реверсовані ще до початку компонування).<br>
...
Для отримання стилів компонування `UIExWrapPanel` використовуйте метод `WrapPanel()` поля StyleLayout.
  ...
  <tr>
    ...
    <td align="left">
      <a 
        ...
        Розділ II: Елементи компонування. Частина 5: «UIExDockPanel»
      </a>
    </td>
  </tr>
  <tr>
